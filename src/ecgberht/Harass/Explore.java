...
public class Explore extends Conditional {
    ...
    public State execute() { // TODO improve
        try {
            ...
            // pending improving logic and behaviour for exploring enemy bases and dodging melee units
            int BaseSize = 10 * 32;
            int Xlocation = 0;
            int Ylocation = 0;
            ...
            Position scoutMainPos = new Position(Xlocation, Ylocation);


            if (((GameState) this.handler).chosenHarasser.getDistance(scoutMainPos) > 2 * 32) {

                ((GameState) this.handler).chosenHarasser.move(scoutMainPos);
                return State.SUCCESS;
